...
const TableDataComp = () => {
  ...
  const customStyles = {
    ...
    option: (styles, { isSelected }) => ({
      ...styles,
      backgroundColor: isSelected ? "lightblue" : "white",
      color: isSelected ? "white" : "#01100A",
      // padding: "8px 14px",
      marginBottom: "6px", // Adjust spacing as needed
      cursor: "pointer",
    }),
    input: (styles) => ({
      ...styles,
    ...
    placeholder: (styles) => ({
      ...styles,
      color: "#01100A",
      opacity: 0.5, // Adjust placeholder opacity as needed
    }),
    ...
    indicatorSeparator: (styles) => ({
      // display: "none",
    }), // Hide default indicator separator
    dropdownIndicator: (styles, { isSelected }) => ({
      ...styles,
      transform: isSelected ? "rotate(180deg)" : "none", // Rotate dropdown icon on selection
    }),
    menu: (styles) => ({
      ...styles,
      padding: 0,
    ...
    optionList: (styles) => ({
      ...
      // overflowY: "auto", // Enable scrolling for long option lists
    }),
  };

  const options = ourServices.map((val) => ({ label: val.label, value: val.label }));
